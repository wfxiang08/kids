...
Master::Master(const KidsConfig *conf) :
                config_ { atoi(conf->worker_threads.c_str()), conf->listen_socket, conf->listen_host, atoi(
                                conf->listen_port.c_str()), conf->ignore_case == "on", atoi(conf->max_clients.c_str()),
                                { conf->nlimit[0], conf->nlimit[1], conf->nlimit[2] }, 1000000 } {
...
Master *Master::Create(const KidsConfig *conf) {
    ...
    if (k->tcpfd_ > 0 && aeCreateFileEvent(k->eventl_, k->tcpfd_, AE_READABLE, AcceptTcpHandler, NULL) == AE_ERR) {
        ...
        LogInfo(
                        "The server is now ready to accept connections on %s:%d", k->config_.listen_host.c_str(), k->config_.listen_port);
    }
